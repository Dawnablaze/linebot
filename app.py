...
CHANNEL_SECRET = os.getenv("CHANNEL_SERECT")
...
def handle_message(event):
    with ApiClient(configuration) as api_client:
        ...
        # 當使用者輸入的文字作為 FAQ 的 key 時，將 value 作為 bot message
        if user_msg in faq:
            ...
            completion = client.chat.completions.create(
                model="gpt-4o-mini",
                messages=[
                    {"role": "system", "content": """你是一個詩人，可以幫使用者把文字主題改成詩歌"""},
                    {
                        "role": "user",
                        "content": "user_msg"
                    }
            ...
            print("AI 的回應", ai_text)
            bot_msg = TextMessage(text=ai_text)

        # if user_msg == "電話":
        #     bot_msg = TextMessage(text="0920123456")
        # elif user_msg == "營業時間":
        #     bot_msg = TextMessage(text="10-18")
        # elif user_msg == "地址":
        ...
        line_bot_api.reply_message_with_http_info(
            ReplyMessageRequest(
                ...
                messages=[
                    # 要回應的內容放在這個串列中
                    TextMessage(text="你好，請輸入 menu 查看選單"),
                    bot_msg
                ]
            )
        )
